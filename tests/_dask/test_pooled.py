...
def test_resolve_pixi_prefix_uses_current_python_prefix(
    ...
    monkeypatch.setattr("fasthep_distributed._dask._worker_env.sys.prefix", str(prefix))
...
def test_resolve_pixi_prefix_errors_for_missing_prefix(
    ...
    monkeypatch.setattr(
        "fasthep_distributed._dask._worker_env.sys.prefix",
...
def test_discover_editable_distributions(
    ...
    monkeypatch.setattr(
        "fasthep_distributed._dask._worker_env.sysconfig.get_path",
...
def test_build_editable_snapshot_installs_offline_and_records_manifest(
    ...
    monkeypatch.setattr(
        "fasthep_distributed._dask._worker_env.subprocess.run",
        fake_run,
    )
    monkeypatch.setattr("fasthep_distributed._dask._worker_env.vcs_state", lambda path: None)
...
def test_prepare_prefix_environment_creates_archive_bootstrap_and_manifest(
    ...
    monkeypatch.setattr("fasthep_distributed._dask._worker_env.sys.prefix", str(prefix))
    monkeypatch.setattr(
        "fasthep_distributed._dask._worker_env.pack_relocatable_prefix",
        lambda prefix, output: output.write_text("archive", encoding="utf-8") or output,
    )
    monkeypatch.setattr(
        "fasthep_distributed._dask._worker_env.discover_editable_distributions",
        lambda prefix: [],
    )
    monkeypatch.setattr(
        "fasthep_distributed._dask._worker_env.required_fasthep_imports",
        lambda prefix, editables: ["distributed", "hepflow"],
    )
    monkeypatch.setattr(
        "fasthep_distributed._dask._worker_env.verify_source_prefix_imports",
        lambda prefix, *, required_imports: None,
    )
    monkeypatch.setattr(
        "fasthep_distributed._dask._worker_env.validate_packed_prefix_archive",
        lambda archive_path, **kwargs: None,
    )
    monkeypatch.setattr(
        "fasthep_distributed._dask._worker_env.installed_package_records",
...
def test_transfer_prefix_environment_snapshots_multiple_editables(
    ...
    monkeypatch.setattr(
        "fasthep_distributed._dask._worker_env.build_editable_snapshot",
        fake_snapshot,
    )
    monkeypatch.setattr(
        "fasthep_distributed._dask._worker_env.validate_packed_prefix_archive",
...
def test_verify_source_prefix_imports_fails_before_submit(
    ...
    monkeypatch.setattr("fasthep_distributed._dask._worker_env.subprocess.run", fake_run)
